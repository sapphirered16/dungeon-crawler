...
class SeededGameEngine:
    ...
    def initialize_full_dungeon(self):
        ...
        # The dungeon should already be generated by the SeededDungeon constructor
        # But we ensure all floors are fully populated with content
        print(f"⚙️  Initializing dungeon content for seed {self.seed}...")
        
        # Make sure the dungeon is fully generated with all content
        # The SeededDungeon should have already generated all rooms, items, enemies, etc.
        # based on the seed during its construction, but we make sure everything is in place
        
        # If needed, we could add any additional initialization here
        # but the seeded generation should already have placed everything deterministically
        
        # Set the player's starting position based on the entrance room
        self.current_room = self._find_starting_room()
        if self.current_room:
            # Position player in the center of the starting room
            center_x = self.current_room.x + self.current_room.width // 2
            center_y = self.current_room.y + self.current_room.height // 2
            self.player.position = (center_x, center_y, self.current_room.z)
