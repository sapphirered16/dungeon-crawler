"""Enemy class with AI behavior for the new dungeon crawler game."""
...
if TYPE_CHECKING:
    from .new_dungeon import SeededDungeon, Room


class Enemy(Entity):
    """Enemy entity with AI behavior for the new dungeon system."""
    ...
    def take_turn(self, current_room: 'Room', dungeon: 'SeededDungeon', player: Player) -> Dict[str, Any]:
        """
        ...
        # In the new system, we check if player position is within the current room
        if current_room and current_room.contains_position(player.position[0], player.position[1]):
            # Attack the player
    ...
    def _attack_player(self, player: Player, current_room: 'Room') -> Dict[str, Any]:
        """Attack the player if in the same room."""
        result = {
            "moved": False,
            "attacked": True,
            "action_description": "",
            "new_position": current_room.get_center() if current_room else None
        }
    ...
    def _hunt_player(self, current_room: 'Room', dungeon: 'SeededDungeon', player: Player) -> Dict[str, Any]:
        ...
        if path_to_player and len(path_to_player) > 1:
            ...
            next_pos = path_to_player[1]  # First step after current position
            
            # Find which room this position belongs to
            next_room = dungeon.get_room_at_position(next_pos)
            
            if next_room:
                result["new_position"] = next_room.get_center()
                result["moved"] = True
    ...
    def _patrol(self, current_room: 'Room', dungeon: 'SeededDungeon') -> Dict[str, Any]:
        """Random patrol movement."""
        result = {
            "moved": False,
            "attacked": False,
    ...
    def _find_path_to_player(self, current_room: 'Room', player: Player, dungeon: 'SeededDungeon') -> List[Tuple[int, int, int]]:
        """Find path to player using BFS."""
        ...
        # In the new system, we need to use the grid-based pathfinding
        # Start from any position in the current room
        start_pos = current_room.get_center()
        target_pos = player.position
        
        if start_pos == target_pos:
            return [start_pos]
        
        # BFS to find shortest path in the dungeon grid
        ...
        while queue:
            ...
            # Check all possible moves (N, S, E, W)
            x, y, z = current_pos
            possible_moves = [
                (x, y-1, z),  # North
                (x, y+1, z),  # South
            ...
            for next_pos in possible_moves:
                # Check if the position is valid (room or hallway)
                cell_type = dungeon.get_cell_type_at_position(next_pos)
                if cell_type in ['room', 'hallway'] and next_pos not in visited:
    ...
    def _is_player_in_line_of_sight(self, current_room: 'Room', player: Player, dungeon: 'SeededDungeon') -> bool:
        """Check if player is in line of sight from current room."""
        if not current_room or not player.position:
            return False
        
        # Check if player is on the same floor
        if current_room.z != player.position[2]:  # Different floors
            return False
        
        # Calculate distance between current room center and player position
        room_center = current_room.get_center()
        dx = abs(room_center[0] - player.position[0])
        dy = abs(room_center[1] - player.position[1])
        ...
        if distance <= max_sight_distance:
            # Check if there's a clear path (through rooms and hallways)
